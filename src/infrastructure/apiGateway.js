...
function mapMonitor(m) {
  return {
    ...
    recoveryThreshold: m.recovery_threshold,
    status: m.status,
...
function mapIncident(i) {
  return {
    ...
    failureCount: i.failure_count,
    recoveryCount: i.recovery_count,
  }
}
...
export function createApiGateway(http) {
  return {
    ...
    reliability: async (days = 30) =>
      (await http.get(`/api/v1/reports/reliability?days=${days}`)).map((r) => ({
        monitorId: r.monitor_id,
        monitorName: r.monitor_name,
        incidentsTotal: r.incidents_total,
        incidentsOpen: r.incidents_open,
    ...
    summary: () =>
      http.get('/api/v1/dashboard/summary').then((s) => ({
        totalMonitors: s.total_monitors,
        up: s.up,
        down: s.down,
        activeIncidents: s.active_incidents,
        uptime24h: s.uptime_24h,
      })),
  }
}
